const routeParams=new URLSearchParams(location.search),routeLeague=routeParams.get('league'),matchRouteId=routeParams.get('match');
const rememberedLeague=localStorage.getItem('spoil-me-not-last-league');
const initialLeague=['all','epl','laliga','ucl','carabao','mlb','nfl'].includes(routeLeague)?routeLeague:['all','epl','laliga','ucl','carabao','mlb','nfl'].includes(rememberedLeague)?rememberedLeague:'epl';
...
let spoilHold=null;
const matchHref=game=>{const url=new URL(location.href);url.searchParams.set('league',game.leagueId);url.searchParams.set('match',game.id);return url.toString()};
...
const mlbStandingRows=(payload,leagueId)=>{
  const seen=new Set();
  return (payload?.records||[]).flatMap(record=>record?.teamRecords||[]).map(entry=>{
    ...
    return {id,name:entry?.team?.name||'',abbr:entry?.team?.abbreviation||entry?.team?.name||'',logo:id?`https://www.mlbstatic.com/team-logos/${id}.svg`:'',rank:standingNumber(entry?.leagueRank??entry?.sportRank??entry?.divisionRank),gp,wins,draws:'',losses,for:entry?.runsScored??'',against:entry?.runsAllowed??'',gd:entry?.runDifferential??'',points:entry?.winningPercentage??'',pct:entry?.winningPercentage??'',gamesBack:entry?.gamesBack??'',leagueId};
...
async function fetchLeagueStandings(league){
  ...
  const request=(async()=>{
    try{
      const endpoint=league.sport==='baseball'?`https://statsapi.mlb.com/api/v1/standings?leagueId=103,104&season=${new Date().getUTCFullYear()}&standingsTypes=regularSeason`:`https://site.api.espn.com/apis/v2/sports/${league.sport}/${league.slug}/standings`,response=await fetch(endpoint),payload=response.ok?await response.json():null,rows=league.sport==='baseball'?mlbStandingRows(payload,league.id):standingRows(payload,league.id);
      if(rows.length){
...
renderRibbon=function(all){
  ...
  const shown=selected.slice(0,8),league=activeLeague==='all'?allLeagueOption:EPLData.leagues[activeLeague];
...
incidentTimeline=function(game){
  const incidentColour=teamId=>{
    const raw=String(teamId)===String(game.homeId)?game.homeColor:String(teamId)===String(game.awayId)?game.awayColor:'';
  ...
  if(game.sport==='baseball'){
    ...
    return scoring.map(event=>{
      ...
      return `<div class="incident run"${incidentStyle(event.teamId)}><time>${inning}</time><span class="event-badge run">${logo?`<img src="${logo}" aria-hidden="true">`:''}</span><span class="event-mark">${score}</span><span>${event.text||'Scoring play'}</span></div>`;
  ...
  return events.map(event=>{
    ...
    return `<div class="incident ${event.type}"${incidentStyle(event.teamId)}><time>${clock}</time>${badge}<span class="event-mark">${marker}</span><span>${label}${rawText||'Scoring play'}</span></div>`;
...
const soccerTeamColor=(group,game,side)=>{
  const raw=String(group?.roster?.team?.color||group?.roster?.team?.alternateColor||group?.roster?.color||(side==='home'?game.homeColor:game.awayColor)||'').replace(/^#/, '').trim();
...
const soccerFormationLineup=(roster,index)=>{
  ...
  const fieldPlayers=[...rows.def,...rows.mid,...rows.att];
  // Providers often label every central player as a midfielder.  When a
  // formation is supplied, its row counts are the authoritative layout.
  const formationRows=explicitLines.length&&explicitLines.reduce((sum,count)=>sum+count,0)===fieldPlayers.length
    ? explicitLines.reduce((result,count)=>{const start=result.cursor;result.rows.push(fieldPlayers.slice(start,start+count));result.cursor+=count;return result},{rows:[],cursor:0}).rows
...
function soccerFormationMarkup(game){
  ...
  const playerMarkup=(entry,side,vertical,rowIndex,rowLength,teamColor)=>{
    ...
    return `<div class="soccer-pitch-player lineup-player starter ${side}" style="--x:${x.toFixed(2)}%;--y:${vertical}%;--team-colour:${lineupEscape(teamColor)};--team-ink:${soccerTeamInk(teamColor)}">`+
      ...
      `<span title="${lineupEscape(name)}">${lineupEscape(shortName)}</span>${events}</div>`;
  };
...
listRow=function(game){
  ...
  const stamp=game.live?'<span class="live-stamp-label">LIVE</span>':game.completed?'?':'';
...
const resultWorkspaceMarkup=(game,{includeScorecard=true}={})=>{
  const tabs=['stats','moments','summary','table','form','lineup'];
  const tab=tabs.includes(game.__resultTab)?game.__resultTab:'stats';
  ...
  const stats=game.sport==='baseball'&&includeScorecard?'<p class="spoiler-empty">The inning scorecard is shown above.</p>':boxScoreMarkup(game)||'<p class="spoiler-empty">Official match statistics are not available for this game.</p>';
...
const scheduleHref=game=>{const url=new URL(location.href);url.searchParams.set('league',game.leagueId);url.searchParams.delete('match');return url.toString()};
const matchPageMarkup=game=>{
  ...
  return `<article class="match-page-card" data-game="${safe(game.id)}"><header class="match-page-top"><a href="${safe(scheduleHref(game))}" data-match-back>← ALL MATCHES</a><span>${game.leagueLogo?`<img src="${safe(game.leagueLogo)}" alt="">`:''}${safe(game.league||'MATCH')}</span></header><section class="match-page-hero"><div class="match-page-team home">${game.homeLogo?`<img src="${safe(game.homeLogo)}" alt="">`:''}<b>${safe(game.home)}</b></div><div class="match-page-final"><strong>${score}</strong><small>FULL TIME</small><time>${safe(date)} · ${safe(time)}</time></div><div class="match-page-team away">${game.awayLogo?`<img src="${safe(game.awayLogo)}" alt="">`:''}<b>${safe(game.away)}</b></div></section>${scorers}<div class="match-page-actions">${highlightMarkup(game)}</div>${resultWorkspaceMarkup(game,{includeScorecard:false})}</article>`;
};
...
matchPage.addEventListener('click',event=>{
  ...
  if(tab==='lineup'){game.__lineupSpoilers=true;renderMatchPage(game);return}
  if(tab==='table'&&!leagueStandings[game.leagueId]){game.__leagueTableLoading=true;renderMatchPage(game);fetchLeagueStandings(EPLData.leagues[game.leagueId]).finally(()=>{game.__leagueTableLoading=false;renderMatchPage(game)});return}
...
listRow=function(game){
  ...
  const anticipation=game.anticipationBreakdown||{};
  const preview=!game.completed&&!game.live?`<small class="match-preview">COMPETITIVENESS ${anticipation.competitiveness??'—'} · CONTEXT ${anticipation.tableContext??'—'} · TIMING ${anticipation.seasonTiming??'—'}</small>`:'';
  const lineupAction=game.completed?`<div class="schedule-card-actions"><button type="button" data-schedule-lineup aria-expanded="${!!game.__showLineup}">${game.__showLineup?'HIDE LINEUP':'SHOW LINEUP'}</button></div>`:'';
  ...
  const formShown=!!game.__showForm||futureFormSpoilers.has(String(game.id));
  const formToggle=!game.completed&&!game.live?`<label class="form-spoiler-toggle"><input type="checkbox" data-form-spoilers aria-label="Show records and standings" ${formShown?'checked':''}><span class="form-spoiler-show">SPOILER</span><span class="form-spoiler-hide">HIDE</span></label>`:'';
  return `<article class="list-game ${state} ${game.completed?'match-page-link':''} ${game.sport==='football'?'football-game':''} ${formShown?'form-spoilers':''}" data-game="${game.id}"><strong aria-hidden="${game.completed?'false':'true'}">${stamp}${leagueStamp}</strong><div class="list-content"><div class="list-teams">${team('home',game.home,game.homeLogo)}${team('away',game.away,game.awayLogo)}</div>${lineupAction}${preview}${details}</div><div class="list-meta"><b>${game.live?'IN PROGRESS':timeAway(game)}</b><small class="match-datetime">${cardDateMarkup(game.time)}</small>${matchday}${formToggle}</div></article>`;
};
listShell.addEventListener('click',event=>{
  ...
  game.__lineupLoading=true;
  rerenderAtCard(card);
  EPLData.enrich(game,{lineup:true})
    .then(()=>hydrateRosterFlags(game))
    .finally(()=>{game.__lineupLoading=false;rerenderAtCard(card)});
},true);
listShell.addEventListener('click',event=>{
