...
listShell.addEventListener('click',event=>{const button=event.target.closest('[data-watch-toggle],[data-lineup],[data-results],[data-form-spoilers]'),card=event.target.closest('[data-game]');if(!button||!card)return;const game=games.find(g=>String(g.id)===String(card.dataset.game));if(!game)return;event.preventDefault();event.stopImmediatePropagation();if(button.matches('[data-watch-toggle]')){if(game.__mwRevealed){game.__mwRevealed=false;game.displayScore=50;plot.render();rerenderAtCard(card)}else{plot.activate(game);rerenderAtCard(card)}return}if(button.matches('[data-form-spoilers]')){game.__showForm=!game.__showForm;rerenderAtCard(card);return}if(button.matches('[data-lineup]')){game.__showLineup=!game.__showLineup;game.__showResults=false;if(game.__showLineup){EPLData.enrich(game,{lineup:true}).then(()=>hydrateRosterFlags(game)).finally(()=>rerenderAtCard(card))}else rerenderAtCard(card);return}game.__showResults=!game.__showResults;game.__showLineup=false;EPLData.enrich(game).finally(()=>rerenderAtCard(card))},true);
// Upcoming records and standings are spoilers. Handle this control directly so
// it cannot be swallowed by the older card-level handlers or move the card.
document.addEventListener('click',event=>{const button=event.target.closest('[data-form-spoilers]'),card=event.target.closest('[data-game]');if(!button||!card?.classList.contains('future'))return;event.preventDefault();event.stopImmediatePropagation();const id=String(card.dataset.game),show=!futureFormSpoilers.has(id),game=games.find(item=>String(item.id)===id);show?futureFormSpoilers.add(id):futureFormSpoilers.delete(id);if(game)game.__showForm=show;card.classList.toggle('form-spoilers',show);button.textContent=show?'HIDE FORM':'SPOIL FORM';button.setAttribute('aria-pressed',String(show))},true);
...
listRow=function(game){
  ...
  const formToggle=!game.completed&&!game.live?`<button class="form-spoiler-toggle" data-form-spoilers aria-pressed="${formShown}">${formShown?'HIDE FORM':'SPOIL FORM'}</button>`:'';
