...
let spoilHold=null;
const cancelSpoilHold=state=>{if(!state)return;cancelAnimationFrame(state.frame);state.button.classList.remove('spoil-hold-active','spoil-hold-complete');state.button.style.removeProperty('--spoil-hold-progress');state.button.dataset.spoilHold='cancelled';requestAnimationFrame(()=>{if(state.button.dataset.spoilHold==='cancelled')delete state.button.dataset.spoilHold})};
listShell.addEventListener('pointerdown',event=>{const button=event.target.closest('.list-game.past [data-results]'),card=event.target.closest('[data-game]');if(!button||!card||(event.pointerType==='mouse'&&event.button!==0))return;const game=games.find(item=>String(item.id)===String(card.dataset.game));if(!game||game.__showResults)return;event.preventDefault();event.stopImmediatePropagation();if(spoilHold)cancelSpoilHold(spoilHold);const state={button,card,game,pointerId:event.pointerId,frame:0,started:performance.now()};spoilHold=state;button.dataset.spoilHold='pending';button.classList.add('spoil-hold-active');button.style.setProperty('--spoil-hold-progress','0%');button.setPointerCapture?.(event.pointerId);const fill=now=>{if(spoilHold!==state)return;const progress=Math.min(1,(now-state.started)/500);button.style.setProperty('--spoil-hold-progress',`${(progress*100).toFixed(1)}%`);if(progress<1){state.frame=requestAnimationFrame(fill);return}button.dataset.spoilHold='complete';button.classList.remove('spoil-hold-active');button.classList.add('spoil-hold-complete');spoilHold=null;game.__showResults=true;game.__showLineup=false;game.__showSummary=false;EPLData.enrich(game).finally(()=>rerenderAtCard(card));setTimeout(()=>rerenderAtCard(card),70)};state.frame=requestAnimationFrame(fill)},true);
...
const soccerResultCard=game=>{
  ...
  return `<div class="soccer-result-card">`+
    ...
    `<div class="soccer-result-scorers"><div class="soccer-goal-list home">${scorers(game.homeId)}</div>${soccerBallIcon}<div class="soccer-goal-list away">${scorers(game.awayId)}</div></div>`+
  `</div>`;
};
...
listRow=function(game){
  ...
  const stamp=game.live?'<span class="live-stamp-label">LIVE</span>':game.completed?(revealed?(game.scoreResult?.watchScore??'—'):calculating?'<span class="score-pending" role="status" aria-label="Calculating Spoil Meter"><i class="score-spinner" aria-hidden="true"></i><em>···</em></span>':'?'):'';
...
async function boot(){try{EPLData.setLeague(activeLeague);const all=prepareGames(await EPLData.load(activeLeague));plot=new Gameplot({games,onSelect:select});plot.setTeamFilter(activeTeams);renderLeagueSwitcher();renderRibbon(all);renderList();loadTeamTable(activeLeague);document.body.classList.add('view-list');plotShell.hidden=true;listShell.hidden=false;document.getElementById('loading').hidden=true;requestAnimationFrame(()=>{document.getElementById('list-now')?.scrollIntoView({block:'center'});const warm=()=>prefetchCompleted(games);window.requestIdleCallback?window.requestIdleCallback(warm,{timeout:1200}):setTimeout(warm,350)});search.oninput=()=>renderTeams(all);search.onfocus=()=>renderTeams(all);Promise.all(lineupCandidates(games).map(game=>EPLData.enrich(game).then(()=>hydrateRosterFlags(game)))).finally(()=>{renderList();decorateCards();refreshTimeUI()})}catch(error){document.getElementById('loading').hidden=true;const box=document.getElementById('error');box.textContent=`LIVE DATA UNAVAILABLE — ${error.message}`;box.hidden=false}}boot()})();
